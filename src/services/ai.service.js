...
exports.processProactiveEvent = processProactiveEvent;
...
const handleAppointmentScheduling = async (userId, message) => {
    try {
        ...
        const matchedContact = contacts.find(contact => {
            ...
            const fuzzy = (0, fastest_levenshtein_1.distance)(full, normalizedQuery) <= 2 ||
                (0, fastest_levenshtein_1.distance)(first, normalizedQuery) <= 2 ||
                (0, fastest_levenshtein_1.distance)(last, normalizedQuery) <= 2;
...
async function processProactiveEvent(userId, eventType, eventData, instructions) {
    const prompt = `
