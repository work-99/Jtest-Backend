...
async function checkAndCreateUser() {
    try {
        ...
        const { rows } = await db_1.default.query('SELECT * FROM users WHERE email = $1', ['vergiegpham@gmail.com']);
        if (rows.length > 0) {
            ...
            const { rows: newUser } = await db_1.default.query('INSERT INTO users (email, name, role) VALUES ($1, $2, $3) RETURNING *', ['vergiegpham@gmail.com', 'Vergie G Pham', 'user']);
            console.log('✅ User created:', newUser[0]);
        }
        // Get the user ID
        const { rows: userRows } = await db_1.default.query('SELECT id FROM users WHERE email = $1', ['vergiegpham@gmail.com']);
        ...
        const { rows: credentials } = await db_1.default.query('SELECT * FROM user_credentials WHERE user_id = $1 AND service = $2', [userId, 'google']);
